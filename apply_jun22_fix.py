"""
apply_jun22_fix.py
Follow-up fixes after apply_jun22.py:
  1. Add the 175 missing 2676-DG products from Jun 22 Furniture Excel
     (mostly new Canvas Fern / Canvas Natural variants).
  2. Retry failed thumbnail downloads sequentially with longer delay
     so Dropbox rate-limiting doesn't kill them again.
"""

import json, os, re, hashlib, shutil, requests, time, openpyxl
from io import BytesIO
from PIL import Image
from collections import Counter

BASE_DIR  = "c:/Users/Lenovo/Downloads/NC - Library Naming/Viewer/NC_Turbo_Viewer"
CAT_PATH  = os.path.join(BASE_DIR, "data/catalogue.json")
THUMB_DIR = os.path.join(BASE_DIR, "static/thumbnails")
FUR_EXCEL = r"C:\Users\Lenovo\Master Image Library Dropbox\Master Image Library\NorthCape Library - Master Excel - Furniture - Jun 22, 2026.xlsx"
THUMBNAIL_SIZE = (400, 400)

WF_COLS  = [f"Wayfair Image {i}"   for i in range(1, 16)]
BY_COLS  = [f"Overstock Image {i}" for i in range(1, 16)]
NC_COLS  = [f"Northcape Image {i}" for i in range(1, 16)]
HD_COLS  = [f"Home Depot Image {i}" for i in range(1, 16)]
ALL_COLS = WF_COLS + BY_COLS + NC_COLS + HD_COLS

stats = {"added": 0, "downloaded": 0, "failed": 0, "retried_ok": 0}

def parse_color(val):
    if not isinstance(val, str): return None, None
    m = re.match(r'=HYPERLINK\("([^"]+)",\s*"([^"]+)"\)', val)
    if m: return m.group(2), m.group(1)
    return (val.strip() or None, None)

def get_priority_urls(p, limit=5):
    urls = []
    for cols in [WF_COLS, BY_COLS, NC_COLS]:
        for col in cols:
            v = (p.get(col) or "")
            v = str(v).strip() if v else ""
            if v.startswith("http"): urls.append(v)
        if len(urls) >= limit: break
    return urls[:limit]

def download_thumb(url, delay=1.5):
    fname = hashlib.md5(url.encode()).hexdigest() + ".jpg"
    dest  = os.path.join(THUMB_DIR, fname)
    if os.path.exists(dest):
        return f"thumbnails/{fname}"
    time.sleep(delay)
    try:
        raw  = url.replace("dl=0", "raw=1").replace("dl=1", "raw=1")
        r = requests.get(raw, timeout=30)
        ct = r.headers.get("Content-Type", "")
        if r.status_code == 200 and "image" in ct:
            img = Image.open(BytesIO(r.content))
            if img.mode in ("RGBA", "P"): img = img.convert("RGB")
            img.thumbnail(THUMBNAIL_SIZE)
            img.save(dest, "JPEG", quality=85)
            return f"thumbnails/{fname}"
    except Exception:
        pass
    return None

# Load catalogue
print("Loading catalogue...", flush=True)
with open(CAT_PATH, encoding="utf-8") as f:
    data = json.load(f)
cat_map = {p["Part Number"]: i for i, p in enumerate(data)}
print(f"Catalogue: {len(data)} products", flush=True)

# Load Furniture Excel
print("Reading Furniture Excel (2676 sheet)...", flush=True)
wb = openpyxl.load_workbook(FUR_EXCEL)
excel_2676 = {}
for sheet in wb.sheetnames:
    if "2676" not in sheet: continue
    ws = wb[sheet]
    headers = [c.value for c in ws[1]]
    if "Part Number" not in headers: continue
    pn_col = headers.index("Part Number") + 1
    for row in ws.iter_rows(min_row=2):
        pn = row[pn_col - 1].value
        if not pn: continue
        excel_2676[pn.strip()] = {headers[i]: (row[i].value if i < len(row) else None) for i in range(len(headers))}

print(f"  Excel 2676 rows: {len(excel_2676)}", flush=True)

# ─── 1. Add missing 2676 products ────────────────────────────────────────────
print("\n=== 1. Adding missing 2676-DG products ===", flush=True)
missing = sorted(set(excel_2676.keys()) - set(cat_map.keys()))
print(f"Missing in catalogue: {len(missing)}", flush=True)

added_indices = []
for n, pn in enumerate(missing, 1):
    exc = excel_2676[pn]
    cn, cl = parse_color(exc.get("Color"))
    new_p = {
        "Part Number":         pn,
        "Collection":          exc.get("Collection") or "2676 - DG",
        "Collection Type":     exc.get("Collection") or "2676 - DG",
        "Category":            "Furniture",
        "Type":                exc.get("Type") or "Furniture",
        "Color":               cn or "",
        "Color_Link":          cl or "",
        "Product":             exc.get("Product") or "",
        "Arm/Table-Top":       exc.get("Arm/Table-Top") or "",
        "Panel":               exc.get("Panel") or "",
        "Dropbox Folder Path": exc.get("Dropbox Folder Path") or "",
        "Last Modified":       str(exc.get("Last Modified") or ""),
        "NC Image Count":      int(exc.get("NC Image Count") or 0),
        "BY Image Count":      int(exc.get("BY Image Count") or 0),
        "WF Image Count":      int(exc.get("WF Image Count") or 0),
        "HD Image Count":      int(exc.get("HD Image Count") or 0),
        "Image_List":          [],
        "Local_Thumbnail":     "",
    }
    for col in ALL_COLS:
        v = (exc.get(col) or "")
        v = str(v).strip() if v else ""
        new_p[col] = v if v.startswith("http") else ""
    # Download priority URLs (sequential with delay to avoid rate-limit)
    urls = get_priority_urls(new_p)
    il = []
    for url in urls:
        r = download_thumb(url, delay=0.8)
        if r:
            il.append(r)
            stats["downloaded"] += 1
        else:
            stats["failed"] += 1
    new_p["Image_List"]      = il
    new_p["Local_Thumbnail"] = il[0] if il else ""
    data.append(new_p)
    added_indices.append(len(data) - 1)
    stats["added"] += 1
    if n % 20 == 0:
        print(f"  [{n}/{len(missing)}] added | dl={stats['downloaded']} fail={stats['failed']}", flush=True)
print(f"Added: {stats['added']} products, downloaded {stats['downloaded']} thumbs, {stats['failed']} failed", flush=True)

# Rebuild cat_map after additions
cat_map = {p["Part Number"]: i for i, p in enumerate(data)}

# ─── 2. Retry failed downloads (products with < 5 images that should have more) ─
print("\n=== 2. Retry failed downloads (sequential, 1.5s delay) ===", flush=True)
retry_targets = []
for idx, p in enumerate(data):
    il = p.get("Image_List", [])
    if len(il) >= 5:
        continue
    urls = get_priority_urls(p)
    have = set(os.path.basename(x) for x in il)
    needed = [u for u in urls if hashlib.md5(u.encode()).hexdigest()+".jpg" not in have]
    slots = 5 - len(il)
    if needed:
        retry_targets.append((idx, p["Part Number"], needed[:slots]))

total_to_retry = sum(len(n) for _, _, n in retry_targets)
print(f"Products needing retry: {len(retry_targets)} | URLs: {total_to_retry}", flush=True)

retry_done = 0
retry_ok = 0
for idx, pn, urls in retry_targets:
    p = data[idx]
    new_il = list(p.get("Image_List", []))
    for url in urls:
        r = download_thumb(url, delay=1.5)
        if r:
            if r not in new_il:
                new_il.append(r)
            retry_ok += 1
        retry_done += 1
        if retry_done % 50 == 0:
            print(f"  [{retry_done}/{total_to_retry}] retry ok={retry_ok}", flush=True)
    p["Image_List"] = new_il[:5]
    p["Local_Thumbnail"] = new_il[0] if new_il else ""

stats["retried_ok"] = retry_ok
print(f"Retry done: {retry_ok}/{total_to_retry} succeeded", flush=True)

# Save
print("\nSaving catalogue...", flush=True)
with open(CAT_PATH, "w", encoding="utf-8") as f:
    json.dump(data, f, indent=2, ensure_ascii=False)

print(f"\n{'='*60}")
print("FOLLOW-UP SUMMARY")
print(f"  New 2676-DG products added:   {stats['added']}")
print(f"  Thumbnails downloaded (new):  {stats['downloaded']}")
print(f"  Initial download failures:    {stats['failed']}")
print(f"  Retry successes:              {stats['retried_ok']}")
print(f"  Final catalogue size:         {len(data)}")

# Final distribution
dist = Counter(len(p.get("Image_List", [])) for p in data)
print("\nImage_List distribution:")
for k in sorted(dist):
    print(f"  {k} images: {dist[k]} products")
